...
def work(item):
        ...
        target_db = mdb.connect(host='bi.zetafin.cn', port=3306, user='root', passwd='wVT6UWcmrH55', charset='utf8')
        ...
        try:
            ...
            while True:
                ...
                if not getstgdata:break 
                    ...
                    target_db.commit()
            update_sql="update report_ddj_bi.synchtodate_new  set start_date =(select ifnull((select DATE_ADD(%s,INTERVAL -3 HOUR)  from %s where id=(select  max(id) from %s)),CURRENT_DATE))   where type='table' and target_table='%s'" %(datetime_column_name,item[6],item[6],item[6])
...
# 目标地址的数据连接
config_db = mdb.connect(host='bi.zetafin.cn', port=3306, user='root', passwd='wVT6UWcmrH55', db='report_ddj_bi', charset='utf8')
config_cur = config_db.cursor()
# 从配置表中查询出来要同步的数据
config_cur.execute("select * from  report_ddj_bi.synchtodate_new where type='table' and status=1")    
